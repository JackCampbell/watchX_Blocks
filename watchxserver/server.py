...
from watchxserver import actions
...
def launch_server(ip='localhost', port=8000, document_root_path=''):
    ...
    :param document_root_path: Path to be the server document root, defualt cwd.
    :return: This function DOES NOT return.
    """
    global document_root
    print('Setting HTTP Server Document Root to:\n\t%s' % document_root_path)
...
def set_header_no_cache():
    ...
    if sys.version_info[0] < 3:
        response.headers['Cache-Control'.encode('ascii', 'ignore')] = 'no-cache'
    else:
...
@app.route('/watchx')
def index_redirect():
    """Redirect the server entry point to the watchx front end."""
    redirect('/watchx/index.html')


@app.route('/watchx/<file_path:path>')
def static_resource(file_path):
    """Serve the 'watchx blocks' folder static files.

    :param file_path: File path inside the 'watchx' resource folder.
    :return: Full HTTPResponse for the static file.
    """
    return static_file(file_path, root=os.path.join(document_root, 'watchx'))
...
def static_closure(file_path):
    ...
    :return: Full HTTPResponse for the static file.
    """
    return static_file(file_path, root=os.path.join(document_root, 'closure-library'))
