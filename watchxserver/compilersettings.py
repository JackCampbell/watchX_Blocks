...
class ServerCompilerSettings(object):
	...
	__arduino_types = {'watchX': 'arduino:avr:leonardo'}
	# 'Uno': 'arduino:avr:uno',
	...
	__ide_load_options = {'open': 'Open sketch in IDE', 'verify': 'Verify sketch', 'upload': 'Compile and Upload sketch'}
	#
	...
	def __initialise(self, settings_dir=None):
		...
		if settings_dir:
			...
			self.__settings_path = os.path.normpath(os.path.join(called_script_dir, self.__settings_filename))
	...
	def check_path(self, absolute_path):
	...
	def set_sketch_name_default(self):
		self.__sketch_name = 'watchxsketch'
	...
	def set_load_ide(self, new_load_option):
		if new_load_option in self.__ide_load_options:
			...
			print('IDE options set to:\n\t%s' % self.__ide_load_options[self.__load_ide_option])
			self.save_settings()
		else:
