...
def parsing_cl_args():
    ...
    if len(sys.argv) == 1:
        ...
        try:
            opts, args = getopt.getopt(sys.argv[1:], 'hs:bf', ['help', 'serverroot=', 'nobrowser', 'findprojectroot'])
        ...
        for opt, arg in opts:
            ...
            if opt in ('-s', '--serverroot'):
                ...
                if re.match("^[a-zA-Z]:$", arg):
                    print('The windows drive letter needs to end in a slash, eg. %s\\' % arg)
                    sys.exit(1)
                ...
                print('Parsed "%s" flag. The watchXBlocks project root will be set as the server root directory.' % opt)
            else:
...
def main():
    ...
    if os.path.isdir(watchxserver.local_packages_path):
        print('Local packages: %s' % watchxserver.local_packages_path)
    else:
    ...
    watchx_root_dir = find_watchx_dir(this_file_dir)
    if watchx_root_dir is None:
        print('The watchX project root folder could not be found within the %s directory !' % this_file_dir)
        sys.exit(1)
    print('watchX root directory:\n\t%s' % watchx_root_dir)
    os.chdir(watchx_root_dir)
    print('Current working directory set to:\n\t%s' % os.getcwd())

    if find_project_root is True or server_root is None:
        server_root = watchx_root_dir
    else:
        # Arguments have set a server root, and to not find ardublockly dir
        if not os.path.commonprefix([server_root, watchx_root_dir]):
            print('The watchXBlocks project folder needs to be accessible from the server root directory !')
    ...
    watchxserver.server.launch_server(ip=SERVER_IP, port=SERVER_PORT, document_root_path=server_root)
    pass
