...
def set_tag(tag):
	...
	"""
	print(script_tag + "Setting the watchXBlocks package tag to '%s'" % tag)
	global copy_dir_name
	global copied_project_dir
	copy_dir_name = "watchXBlocks_%s" % tag
	# In OSX everything goes into the folder path Ardublockly.app/Contents/
	if platform.system() == "Darwin":
		copy_dir_name = os.path.join(copy_dir_name, 'watchXBlocks.app', 'Contents')
...
def get_build_tag():
	...
	arch_time_stamp = "%s%s_%s" % (platform.system(), (struct.calcsize('P') * 8), time.strftime("%Y-%m-%d_%H.%M"))
...
def zip_watchx_blocks_copy(name_append):
	"""
	...
	zip_file_location = os.path.join( zip_file_dir, "watchXBlocks_%s.zip" % name_append )
	...
	# In OS X copied_project_dir is watchXBlocks_tag/watchXBlocks.app/Contents/
...
def pack_watchx_blocks(tag):
	...
	print(script_tag + "Removing any already zipped watchXBlocks version:")
	remove_directory(os.path.join(copied_project_dir, "releases"))

	print(script_tag + "Removing Electron session app data files:")
	remove_directory(os.path.join(copied_project_dir, "watchx_exec", "appdata"))
...
def main():
	print(script_tag + "Pack watchX Blocks script started.")
