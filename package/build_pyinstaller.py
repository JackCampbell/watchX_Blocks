...
def get_os():
	...
	if len(sys.argv) > 1:
		if sys.argv[1] in valid_os:
			...
			print(script_tab + "Valid command line argument found: %s" % sys.argv[1])
			return "%s" % sys.argv[1]
		else:
			print(script_tab + "Invalid command line argument found: %s" % sys.argv[1])
...
def pyinstaller_build():
	...
	"""
	process_args = ["pyinstaller", "%s" % os.path.join("package", "pyinstaller.spec")]
	...
	if pyinstaller_process.returncode != 0:
		print(script_tab + "ERROR: PyInstaller returned with exit code: %s" % pyinstaller_process.returncode)
		return False
...
def move_executable_folder():
	...
	if os.path.exists(original_exec_dir):
		...
		print(script_tab + "ERROR: PyInstaller executable output folder '%s' not found!" % original_exec_dir)
		return False
	return True
...
def build_ardublockly():
	...
	if not success:
		print(script_tab + "Removing PyInstaller recent temp directories.")
		remove_pyinstaller_temps()
		raise SystemExit(script_tab + "Exiting now as there was an error in the PyInstaller execution.")
