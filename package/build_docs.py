...
def pull_wiki_repo():
	...
	if std_err_op:
		print("ERROR: Could not get the remote information from the wiki repository !\n%s" + std_err_op)
		return False

	if GITHUB_WIKI_REPO not in str(std_op):
		print("ERROR: Wiki repository: %s\nnot found in directory %s: \n%s" % (GITHUB_WIKI_REPO, WIKI_DIR, std_op))
		return False
	...
	with open(temp_abs_path, 'w') as temp_file:
		with open(mkdocs_yml) as original_file:
			for line in original_file:
				...
				print("Did not find the 'pages' property in mkdocs.yml file. Attaching the property at the end.")
...
def generate_pages_from_sidebar():
	...
	for line in sidebar_data:
		yml_sections.append(line.replace("[", "").replace("](", ": '").replace(")", ".md'"))
...
def build_mkdocs():
	...
	pipe = subprocess.PIPE
	mkdocs_process = subprocess.Popen(["mkdocs", "build", "--clean"], stdout=pipe, stderr=pipe)
