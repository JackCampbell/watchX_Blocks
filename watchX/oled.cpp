...
*/
void drawString_P(uint8_t x,uint8_t y,const char*s,const char* font,char startindex){
   char c;
if(!s)return;
...
void drawPixel(uint8_t x, uint8_t y,char color) {
...
void ssd1306_spiDataStart()
{
  ...
  byte readPixels(const char* loc, bool invert)
{
...
void draw_bitmap(byte x, byte yy, const char* bitmap, byte w, byte h, bool invert, byte offsetY)
{
